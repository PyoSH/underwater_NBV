...
class NBVBROVEnvCfg(DirectRLEnvCfg):
    ...
    # ── 임계값 하향(구제) — 2026-09-07 추가 ────────────────────────────
    # 성공률 EMA가 `relief_gate` 아래로 떨어지면 임계값을 다시 내린다.
    #
    # 왜 필요한가 (실측): 2026-09-07 런에서 임계값이 상한 0.92에 도달했는데
    # 정책의 최고 coverage는 0.886이었다. 25결정 예산에서 0.92는 도달 불가라
    # **성공률이 구조적으로 0**이 됐고, `coverage_bonus`(리턴의 절반)가 사라져
    # 리턴이 31→13.5로 반토막 났으며, 이후 23롤아웃 동안 PPO 갱신이 0회였다.
    # 단조 증가 규칙만 있으면 이 상태에서 **영구히 회복 불가**다.
    #
    # 단조 규칙의 원래 목적(보상 분포 비정상화로 critic이 흔들리는 것 방지)은
    # 여전히 유효하므로 폐기하지 않고 **넓은 사구간(dead band)** 으로 남긴다:
    # 0.7 이상이면 올리고, 0.10 이하면 내리며, 그 사이에서는 고정. 진동하려면
    # 성공률이 한 사구간을 통째로 가로질러야 해서 실질적으로 일어나지 않는다.
    #
    # 하향이 상향보다 빠른 이유: 죽은 상태에서 빠져나오는 것이 난이도를 올리는
    # 것보다 급하다. 올라갈 기회는 계속 오지만, 못 빠져나오면 런 전체를 잃는다.
    curriculum_relief_gate: float = 0.10
    curriculum_relief_rate: float = 0.010   # 상향(0.005)의 2배
    ...
    # ── 자기중심 voxel 관측 (2026-09-10) ────────────────────────────────
    # voxel 격자는 **물체중심 월드 프레임**(축정렬)인데 행동은 **에이전트 구면
    # 프레임**(dtheta,dphi,dpsi)이다. "+X가 미관측" → "theta를 0쪽으로"라는
    # 좌표변환을 conv+MLP가 스스로 배워야 하는데, actor는 theta를 스칼라로만
    # 받으므로 그 스칼라를 조건으로 3차원 격자를 회전시키는 연산을 학습해야 한다.
    #
    # 그게 병목이라는 실측 근거(run08, 2026-09-09 절제 시험):
    #   정상 지도 0.782 / **다른 env 지도로 교체 0.802** / 지도 제거 0.439.
    # 지도가 없으면 무너지므로 정책은 voxel에 의존하지만, 남의 지도로 바꿔도
    # 성능이 같으므로 쓰는 것은 공간적 대응이 아니라 진행도 신호뿐이다.
    #
    # True면 격자를 에이전트 방위각만큼 돌려 넣는다 — 변환이 이미 되어 있어
    # "내 앞쪽이 미관측"이 격자 위치로 직접 드러난다. 판정: 이 관측으로 학습한
    # 정책이 **지도 섞기에서 성능이 떨어지면** 공간 정보를 쓰기 시작한 것이다.
    vox_egocentric: bool = False
