#!/usr/bin/env bash
# 자세 유지 시험 (2026-09-10): run_case_c_deploy_model_based.sh 를 물려받되
# (1) 미션/컨트롤러 설정을 인자로 받고 (2) 코너 순회 감독 대신 정해진 시간 동안
# 종점 자세 유지를 기록한다. NBV look-at 자세(pitch ±80°)를 model-based PID가
# 유지하는지 Gazebo truth / DVL-EKF 피드백에서 잰다.

set -euo pipefail

if [[ $# -ne 5 ]]; then
  echo "usage: $0 <run_dir(attitude_hold_*)> gazebo_truth|mavlink_ekf <mission.yaml> <controller.yaml> <hold_s>" >&2
  exit 2
fi
RUN_DIR=$1
FEEDBACK_SOURCE=$2
...
HOLD_S=$5
case "$RUN_DIR" in
  /home/bluerov2_sitl/brov_ros2/runtime/experiments/attitude_hold_*) ;;
  *) echo "RUN_DIR must be an explicit persistent attitude_hold_* path" >&2; exit 2 ;;
esac
...
MISSION=$MISSION_ARG
CONTROLLER_CONFIG=$CONTROLLER_ARG
ROS_DOMAIN_ID=42
...
cleanup() {
  echo "[attitude-hold-$FEEDBACK_SOURCE] stopping control and task-owned processes"
...

{
  ...
  echo "mission_contract=direct_relative_attitude_hold_1m"
  echo "hold_s=$HOLD_S"
  ...
  sha256sum "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
    ...
    "$BROV_SOURCE/brov_bringup/launch/sim2sim_attitude_hold_model_based.launch.py"
...
GZ_PID=$!
echo "[attitude-hold-$FEEDBACK_SOURCE] Gazebo PID=$GZ_PID"
sleep 1
...
setsid ros2 launch brov_bringup sim2sim_attitude_hold_model_based.launch.py \
...
ros2 param get /brov_obs_node fixed_attitude_enabled >"$RUN_DIR/fixed_attitude_enabled.txt"
ros2 param get /brov_obs_node fixed_attitude_rpy_rad >"$RUN_DIR/fixed_attitude_rpy_rad.txt"
grep -qi "true" "$RUN_DIR/fixed_attitude_enabled.txt"
...
echo "[attitude-hold-$FEEDBACK_SOURCE] preflight passed; arming"
ARMED=false
...
# 케이스 A/C의 GT 상승 배리어(-6.2 m 통과 대기)는 쓰지 않는다. 2026-09-10 실측:
# 현재 Gazebo 기체 모델은 거의 중성부력(자유 상승 0.004 m/s, 8-17 런은 0.1 m/s)이라
# 8 s ARM→START 창 안에 -8.7→-6.2 m 상승이 불가능하고, 이 시험은 시작 깊이의
# 결정성이 필요 없다. 대신 2 s 안정 후 GT 상태를 기록해 둔다.
sleep 2
timeout 2 ros2 topic echo --once /brov/sim/gazebo_odometry_raw \
  >"$RUN_DIR/gt_at_start.txt" || true
...
echo "[attitude-hold-$FEEDBACK_SOURCE] active; holding for ${HOLD_S}s"
sleep "$HOLD_S"
SUPERVISOR_PID=
...
echo "[attitude-hold-$FEEDBACK_SOURCE] hold completed"
