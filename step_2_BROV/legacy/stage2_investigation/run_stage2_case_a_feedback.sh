#!/usr/bin/env bash
# Fresh full Case-A cycle using either Gazebo truth or Water Linked VPD/EKF.

set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "usage: $0 /tmp/stage2_case_a_<run_id> gazebo_truth|mavlink_ekf" >&2
  exit 2
fi
RUN_DIR=$1
FEEDBACK_SOURCE=$2
case "$RUN_DIR" in
  /tmp/stage2_case_a_*) ;;
  *) echo "RUN_DIR must be an explicit /tmp/stage2_case_a_* path" >&2; exit 2 ;;
esac
...
MISSION=/tmp/stage2_case_a_0p5_2m.yaml
...
POLICY=$BROV_SOURCE/artifacts/policies/demo_policy/policy.pt
ROS_DOMAIN_ID=42
...
cleanup() {
  echo "[case-a-$FEEDBACK_SOURCE] stopping control and task-owned processes"
...
for required in "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
  "$ORIGIN_HELPER" "$CYCLE_SUPERVISOR" "$START_BARRIER" "$POLICY"; do
  test -f "$required"
done
...
cp "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
  "$CYCLE_SUPERVISOR" "$START_BARRIER" "$RUN_DIR/"

setsid gz sim -s -r -v 2 "$WORLD" >"$RUN_DIR/gazebo.log" 2>&1 &
GZ_PID=$!
echo "[case-a-$FEEDBACK_SOURCE] Gazebo PID=$GZ_PID"
sleep 1
...
# Fail closed if any requested no-GPS/Water Linked EKF parameter was rejected
# or silently fell back to a firmware default.
# MAVProxy writes mav.parm only after its full parameter FTP transfer.
...
setsid ros2 bag record -o "$RUN_DIR/bag" \
  /clock /brov/sim/gazebo_odometry_raw \
...
setsid ros2 launch brov_bringup rl_demo.launch.py \
  vehicle_config:="$BROV_SOURCE/brov_base/config/vehicle_sitl.yaml" \
  mission_file:="$MISSION" \
  safety_config:="$BROV_SOURCE/brov_base/config/safety.yaml" \
  controller_config:="$BROV_SOURCE/brov_control/config/rl_controller.yaml" \
...
# Subscribe before activation so the DVL freshness supervisor already owns a
# valid heartbeat when /brov/control_active rises.
...
echo "[case-a-$FEEDBACK_SOURCE] preflight passed; arming"
ARMED=false
...
# Both feedback arms cross the same physical state before START.  The vehicle
# is positively buoyant and rises monotonically while output remains neutral.
/usr/bin/python3 "$START_BARRIER" --target-world-z -6.2 --timeout-s 6 \
...
if grep -Eq "success=(True|true)|success: true" "$RUN_DIR/start_service.txt"; then
  ...
  # The service callback can activate control even when the CLI response is
  # lost or delayed.  Treat the published lifecycle state as authoritative.
...
echo "[case-a-$FEEDBACK_SOURCE] active; waiting for 0->1->2->1"
wait "$SUPERVISOR_PID"
SUPERVISOR_PID=
...
echo "[case-a-$FEEDBACK_SOURCE] one full cycle completed"
