#!/usr/bin/env python3
"""Wait for one debounced Case-C square-loop cycle (0->1->2->3->0).

Mirrors stage2_wait_case_a_cycle.py exactly, generalized from Case-A's
takeoff_then_align 3-point 0->1->2->1 sequence to Case-C's plain
random_at_waypoint 4-point loop, which advances by simple modulo wrap
(guidance.py's LOSGuidance._current_and_next: next_idx=(idx+1)%num_wp for
any heading_mode other than takeoff_then_align) -- one full lap visits all
four corners and returns to corner 0.
"""
...
class CycleSupervisor(Node):
    """Recognize the looped random_at_waypoint index sequence 0->1->2->3->0."""

    def __init__(self, timeout_s: float, debounce_samples: int) -> None:
        super().__init__("brov_stage2_case_c_cycle_supervisor")
    ...
    def _index(self, message: Int32) -> None:
        ...
        if self.sequence and value == self.sequence[-1]:
            return
        expected = (
            self.expected[len(self.sequence)] if len(self.sequence) < 5 else None
        )
        if value != expected:
            self.failed = (
