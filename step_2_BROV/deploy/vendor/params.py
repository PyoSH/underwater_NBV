"""
BROV2 파라미터 로더 — deploy/vendor/brov2_heavy.yaml의 단일 진입점 (vendored 사본)
====================================================================================
원본은 `robots/dynamics/brov2/params.py` — `deploy/` 폴더 하나만 복사해도 동작하도록
`deploy/vendor/`에 필요한 최소 파일만 내장(vendoring)한 사본이다. 원본이 바뀌면
`deploy/sync_vendor.sh`로 재동기화할 것 — 단일 정본은 여전히 `robots/` 쪽이다.
