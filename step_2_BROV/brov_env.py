"""
BlueROV2 heavy environment for IsaacLabs
...
from .assets.brov import BROV_CFG
...
from .BROV_thruster_dynamics import DynamicsFirstOrder, ConversionFunctionBasic, get_thruster_com_and_orientations

class BROVEnvWindow(BaseEnvWindow):
    """Window manager for the brovenv environment."""

    def __init__(self, env: BROVEnv, window_name: str = "IsaacLab"):
...
@configclass
class BROVEnvCfg(DirectRLEnvCfg):
    ui_window_class_type = BROVEnvWindow

    sim: SimulationCfg = SimulationCfg(dt=1 / 120)

    # robot
    robot_cfg: RigidObjectCfg = BROV_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class BROVEnv(DirectRLEnv):
    cfg: BROVEnvCfg

    def __init__(self, cfg: BROVEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # fake inertial values for BlueROV2 heavy, based on I_ii = (1/12) * mass * (len_j**2 + len_k**2)
        self.inertia_tensors[:, 0] = 0.16
        self.inertia_tensors[:, 1] = 0.22
        self.inertia_tensors[:, 2] = 0.22
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cfg.use_boundaries:
            out_of_bounds = (
                (torch.abs(self._robot.data.root_pos_w[:, 0] - self.scene.env_origins[:, 0]) > self.cfg.max_rov_x) | 
                (torch.abs(self._robot.data.root_pos_w[:, 1] - self.scene.env_origins[:, 1]) > self.cfg.max_rov_y) | 
                (torch.abs(self._robot.data.root_pos_w[:, 2] - self.cfg.starting_depth) > self.cfg.max_rov_z)
            )
        else:
    ...
    def _compute_dynamics(self, actions) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        thruster_forces = thruster_forces * motorValues.unsqueeze(-1) # make motorValues shape (num_envs, 8, 1))
...
def _compute_rewards(
    ...
    # Uniquefy and normalize all quaternions
    rew_ang = rew_scale_ang * torch.exp(-1 * math_utils.quat_error_magnitude(goal[:,:], root_quat[:,:])**2)
