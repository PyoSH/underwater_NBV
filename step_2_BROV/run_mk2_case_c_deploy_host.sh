#!/usr/bin/env bash
# Build/sync the MK2 ROS overlay, then run one fresh Gazebo Case-C-shaped cycle.
...
run_dir="$target_repo/runtime/experiments/sim2sim_mk2_case_c_${run_tag}_${feedback_source}"
...
docker exec -w "$target_repo" "$container_name" bash -lc '
  ...
  ros2 launch brov_bringup sim2sim_mk2_case_c.launch.py --show-args >/dev/null
'
...
docker exec -i -e POLICY_ARTIFACT_ID="$policy_artifact_id" "$container_name" \
  bash "/tmp/run_mk2_case_c_deploy.sh" "$run_dir" "$feedback_source"
