...
# run_mk2_case_a_deploy.sh but swaps policy_node_mk2 for
...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 /home/bluerov2_sitl/brov_ros2/runtime/experiments/case_a_model_based_<run_id> gazebo_truth|mavlink_ekf" >&2
  exit 2
fi
RUN_DIR=$1
FEEDBACK_SOURCE=$2
case "$RUN_DIR" in
  /home/bluerov2_sitl/brov_ros2/runtime/experiments/case_a_model_based_*) ;;
  *) echo "RUN_DIR must be an explicit persistent case_a_model_based_* path" >&2; exit 2 ;;
esac
...
cleanup() {
  echo "[case-a-model-based-$FEEDBACK_SOURCE] stopping control and task-owned processes"
...

{
  ...
  sha256sum "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
    ...
    "$BROV_SOURCE/brov_bringup/launch/sim2sim_case_a_model_based.launch.py"
...
GZ_PID=$!
echo "[case-a-model-based-$FEEDBACK_SOURCE] Gazebo PID=$GZ_PID"
sleep 1
...
setsid ros2 launch brov_bringup sim2sim_case_a_model_based.launch.py \
...
echo "[case-a-model-based-$FEEDBACK_SOURCE] preflight passed; arming"
ARMED=false
...
echo "[case-a-model-based-$FEEDBACK_SOURCE] active; waiting for 0->1->2->1"
wait "$SUPERVISOR_PID"
SUPERVISOR_PID=
...
echo "[case-a-model-based-$FEEDBACK_SOURCE] one full cycle completed"
