...
run_dir="$target_repo/runtime/experiments/case_c_model_based_${run_tag}_${feedback_source}"
...
declare -a support_files=(
  ...
  run_case_c_deploy_model_based.sh
)
...
docker exec -w "$target_repo" "$container_name" bash -lc '
  ...
  ros2 launch brov_bringup sim2sim_case_c_model_based.launch.py --show-args >/dev/null
'

echo "[case-c-model-based-host] fresh run: $run_dir"
docker exec -i "$container_name" bash "/tmp/run_case_c_deploy_model_based.sh" \
  "$run_dir" "$feedback_source"
echo "[case-c-model-based-host] completed: $run_dir"
