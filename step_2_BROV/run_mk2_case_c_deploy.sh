...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 /home/bluerov2_sitl/brov_ros2/runtime/experiments/sim2sim_mk2_case_c_<run_id> gazebo_truth|mavlink_ekf" >&2
  exit 2
fi
RUN_DIR=$1
FEEDBACK_SOURCE=$2
case "$RUN_DIR" in
  /home/bluerov2_sitl/brov_ros2/runtime/experiments/sim2sim_mk2_case_c_*) ;;
  *) echo "RUN_DIR must be an explicit persistent sim2sim_mk2_case_c_* path" >&2; exit 2 ;;
esac
...
cleanup() {
  echo "[mk2-case-c-$FEEDBACK_SOURCE] stopping control and task-owned processes"
...

{
  ...
  echo "mission_contract=direct_relative_case_c_square_5m_0p5"
  ...
  sha256sum "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
    ...
    "$BROV_SOURCE/brov_bringup/launch/sim2sim_mk2_case_c.launch.py" \
    "$BROV_SOURCE/brov_control/config/rl_controller_mk2_deploy_v2.yaml"
...
GZ_PID=$!
echo "[mk2-case-c-$FEEDBACK_SOURCE] Gazebo PID=$GZ_PID"
sleep 1
...
setsid ros2 launch brov_bringup sim2sim_mk2_case_c.launch.py \
...
echo "[mk2-case-c-$FEEDBACK_SOURCE] preflight passed; arming"
ARMED=false
...
echo "[mk2-case-c-$FEEDBACK_SOURCE] active; waiting for 0->1->2->3->0"
wait "$SUPERVISOR_PID"
SUPERVISOR_PID=
...
echo "[mk2-case-c-$FEEDBACK_SOURCE] one full cycle completed"
