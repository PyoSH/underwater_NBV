...
FEEDBACK_SOURCE=$2
case "$RUN_DIR" in
  /home/bluerov2_sitl/brov_ros2/runtime/experiments/sim2sim_mk2_case_a_*) ;;
  *) echo "RUN_DIR must be an explicit persistent sim2sim_mk2_case_a_* path" >&2; exit 2 ;;
esac
...
cleanup() {
  echo "[mk2-case-a-$FEEDBACK_SOURCE] stopping control and task-owned processes"
...

{
  ...
  sha256sum "$WORLD" "$PARAMS" "$MISSION" "$DVL_INJECTOR" \
    ...
    "$BROV_SOURCE/brov_bringup/launch/sim2sim_mk2_case_a.launch.py" \
...
GZ_PID=$!
echo "[mk2-case-a-$FEEDBACK_SOURCE] Gazebo PID=$GZ_PID"
sleep 1
...
setsid ros2 launch brov_bringup sim2sim_mk2_case_a.launch.py \
...
echo "[mk2-case-a-$FEEDBACK_SOURCE] preflight passed; arming"
ARMED=false
...
echo "[mk2-case-a-$FEEDBACK_SOURCE] active; waiting for 0->1->2->1"
wait "$SUPERVISOR_PID"
SUPERVISOR_PID=
...
echo "[mk2-case-a-$FEEDBACK_SOURCE] one full cycle completed"
